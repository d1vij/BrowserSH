...
export function getCommandContext(tokens: Tokens): ExtractionResults {
    ...
    for (let idx = 0; idx < tokens.length; idx++) {
        ...
        if (currTok.length === 2 && currTok[0] === Characters.SingleDash && !test_isNumeric(currTok)) {
            // current token is a flag
            flags.push(currTok[1]);
        }
        else if (currTok.length >= 3 && currTok.startsWith(Characters.DoubleDash, 0)) {
            ...
            if (nextTok.startsWith(Characters.DoubleDash, 0)
                || (nextTok.length === 2 && nextTok[0] === Characters.SingleDash && !test_isNumeric(nextTok))) throw new IncorrectOptionUsageInCommandError(`No option provided to option ${addColor(optionName, Colors.yellow_light)}`);
